...
//  * SearchBar Component
//  *
//  * Real-time search for freelancers across crew directory
//  * Shows dropdown with results after 2+ characters typed
//  * Navigates to freelancer profile on selection
//  */
...
//       try {
//         // Build search endpoint based on scope
//         let searchUrl;
//         if (scope === "skill" && departmentSlug && skillSlug) {
//           searchUrl = `/api/search/freelancers?q=${encodeURIComponent(
//             searchTerm
//           )}&department=${departmentSlug}&skill=${skillSlug}`;
//         } else if (scope === "department" && departmentSlug) {
//           searchUrl = `/api/search/freelancers?q=${encodeURIComponent(
//             searchTerm
...
//   }, [searchTerm, scope, departmentSlug, skillSlug]);
...
//     router.push(`/my-account/${freelancer.slug}`);
//     setSearchTerm("");
//     setShowDropdown(false);
//     setSelectedIndex(-1);
//   };
 ...
 * Real-time search for freelancers across crew directory
 * Shows dropdown with results after 2+ characters typed
 * Opens FreelancerModal on selection (via callback)
 */
export default function SearchBar({
  scope = "all",
  departmentSlug = null,
  skillSlug = null,
  onSelectFreelancer = null, // NEW: Callback to open modal
}) {
  const router = useRouter();
  const [searchTerm, setSearchTerm] = useState("");
  const [results, setResults] = useState([]);
  ...
  // Handle freelancer selection - open modal or navigate
  const handleSelectFreelancer = async (freelancer) => {
    // If callback provided (crew directory page), use it to open modal
    if (onSelectFreelancer) {
      // Need to fetch full freelancer data for modal
      try {
